                              ...
                              BTLeafNode& sibling, int& siblingKey)
{
	...
	sibling.numOfKeys = getKeyCount() - numHalfKeys;
...
int BTNonLeafNode::getKeyCount()
{
	...
	{
		int insideKey;
		std::memcpy(&insideKey, temp, sizeof(int)); //Save the current key inside buffer as insideKey
		if(insideKey==0) //Once we hit key of 0, we break
			break;
		//Otherwise, increment count
		count++;
...
RC BTNonLeafNode::insert(int key, PageId pid)
{
	...
	std::memcpy(newBuffer+i, &key, sizeof(int));
	std::memcpy(newBuffer+i+sizeof(int), &pid, sizeof(PageId));
	...
	std::memcpy(newBuffer+i+pairSize, buffer+i, getKeyCount()*pairSize - i + 8);
	
	
	//Copy newBuffer into buffer, then delete temporary newBuffer to prevent memory leak
	std::memcpy(buffer, newBuffer, PageFile::PAGE_SIZE);
	free(newBuffer);
...
RC BTNonLeafNode::insertAndSplit(int key, PageId pid, BTNonLeafNode& sibling, int& midKey)
{
	...
	std::memcpy(&key1, buffer+halfIndex-8, sizeof(int));
	std::memcpy(&key2, buffer+halfIndex, sizeof(int));
	
	if(key < key1) //key1 is the median key to be removed
	{
		//Copy everything on the right side of halfIndex into sibling's buffer (ignore the pid)
		//For nonleaf nodes only, remember to add an offset of 8 for initial pid
		std::memcpy(sibling.buffer+8, buffer+halfIndex, PageFile::PAGE_SIZE-halfIndex);
		//Update sibling's number of keys
		sibling.numOfKeys = getKeyCount() - numHalfKeys;
		
		//Copy down the median key before getting rid of it in buffer
		std::memcpy(&midKey, buffer+halfIndex-8, sizeof(int));
		
		//Also set the sibling pid from buffer before getting rid of it
		std::memcpy(sibling.buffer, buffer+halfIndex-4, sizeof(PageId));
	...
	else if(key > key2) //key2 is the median key to be removed
	{
		...
		//For nonleaf nodes only, remember to add an offset of 8 for initial pid
		std::memcpy(sibling.buffer+8, buffer+halfIndex+8, PageFile::PAGE_SIZE-halfIndex-8);
		//Update sibling's number of keys
		sibling.numOfKeys = getKeyCount() - numHalfKeys - 1;
		
		//Copy down the median key before getting rid of it in buffer
		std::memcpy(&midKey, buffer+halfIndex, sizeof(int));
	...
	else //key is the median key to be removed
	{
		...
		std::memcpy(sibling.buffer, &pid, sizeof(PageId));
	}
...
RC BTNonLeafNode::initializeRoot(PageId pid1, int key, PageId pid2)
{
	...
	std::memcpy(temp, &pid1, sizeof(PageId));
	
	//Copy the first pair into buffer
	//std::memcpy(temp+8, &key, sizeof(int));
	//std::memcpy(temp+12, &pid2, sizeof(PageId));
